...
function initInfiniteCarousel(carouselSelector, scrollSpeed) {
    ...
    function animateScroll() {
        if (!isPaused) {
            ...
            const lastOriginalItemRight = originalItems[originalItems.length - 1].getBoundingClientRect().right;

            // Check if the last original item is fully out of view and reset scroll position
            if (totalScroll >= singleCycleWidth && lastOriginalItemRight <= 0) {
                totalScroll = 0; // Reset to align the clones perfectly
            }
